	...
	<form action="user/query" method="POST"
		...
		<div class="formbody">
			<div id="usual1" class="usual">
				<ul class="seachform1">
					<li><label>员工姓名</label><input type="text" class="scinput1"
			...
			<table class="tablelist">
				...
				<tbody>
					<c:if test="${message ne null }" var="null">
						<tr>
							<td colspan="15" align="center">${message }</td>
						</tr>
					</c:if>
					<c:forEach items="${pager.datas }" var="user" varStatus="vg">
						<tr>
							...
							<td>
								<c:if test="${user['selfId'] eq 1}">
									<c:out value="男" />
								</c:if>
								<c:if test="${user['selfId'] eq 2}">
									<c:out value="女" />
								</c:if>
							</td>
							<td><c:out value="${user['createDate'] }"></c:out></td>
							<td><c:out value="${user['remark'] }"></c:out></td>
							<td><c:out value="${user['house_phone'] }"></c:out></td>
							<td><c:out value="${user['email'] }"></c:out></td>
							<td><c:out value="${user['address'] }"></c:out></td>
							<td><c:out value="${user['education'] }"></c:out></td>
							<td><c:out value="${user['recommend']['name'] }"></c:out></td>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="user/query?pageNow=${pager.pageNow }&userName=${user['userName']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="user/query?pageNow=${pager.pageNow-1 }&userName=${user['userName']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="user/query?pageNow=${vs.count }&userName=${user['userName']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="user/query?pageNow=${count }&userName=${user['userName']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="user/query?pageNow=${count }&userName=${user['userName']}">${count}</a></li>
